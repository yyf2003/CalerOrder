...
namespace Common
{
    public class CommonMethod
    {
        ...
        public static List<EnumEntity> GetEnumList<T>()
        {
            List<EnumEntity> list = new List<EnumEntity>();
            foreach (var ele in Enum.GetValues(typeof(T)))
            {
                EnumEntity entity = new EnumEntity();
                object[] objArr = ele.GetType().GetField(ele.ToString()).GetCustomAttributes(typeof(DescriptionAttribute), true);
                if (objArr != null && objArr.Length > 0)
                {
                    DescriptionAttribute da = objArr[0] as DescriptionAttribute;
                    entity.Desction = da.Description;
                }
                entity.Value = Convert.ToInt32(ele);
                entity.Name = ele.ToString();
                list.Add(entity);  
            }
            return list;
        }

        public static void DestoryCookie(string cookieName)
        {
            HttpCookie conditionCookie = HttpContext.Current.Request.Cookies[cookieName];
