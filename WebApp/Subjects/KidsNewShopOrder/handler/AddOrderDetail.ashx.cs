...
namespace WebApp.Subjects.KidsNewShopOrder.handler
{
    /// <summary>
    ...
    public class AddOrderDetail : IHttpHandler
    {
            ...
            if (!string.IsNullOrWhiteSpace(shopNo))
            {
                ...
                if (shopModel != null)
                {
                    ...
                    if (isKids)
                    {
                        ...
                        //获取这个店铺已下完的订单
                        var orderList = new HandMadeOrderDetailBLL().GetList(s => s.SubjectId == subjectId && s.ShopId == shopId);
                        string newShopGraphicNo = "signage";
                        var popList = new POPBLL().GetList(s => s.ShopId == shopId && s.GraphicNo != null && s.GraphicNo.ToLower().Contains(newShopGraphicNo));
                        if (popList.Any())
                        {
                            ...
                            popList.ForEach(s =>
                            {
                                ...
                                if (canWork)
                                {
                                    ...
                                    popJson.Append("{\"Id\":\"" + s.Id + "\",\"ShopId\":\"" + s.ShopId + "\",\"Sheet\":\"" + s.Sheet + "\",\"GraphicNo\":\"" + s.GraphicNo + "\",\"Gender\":\"" + s.Gender + "\",\"Quantity\":\"" + s.Quantity + "\",\"GraphicWidth\":\"" + s.GraphicWidth + "\",\"GraphicLength\":\"" + s.GraphicLength + "\",\"GraphicMaterial\":\"" + s.GraphicMaterial + "\",\"PositionDescription\":\"" + s.PositionDescription + "\",\"Remark\":\"" + s.Remark + "\",\"IsValid\":\"" + isValid + "\",\"OrderType\":\"1\"},");
                                }
                            });
                            if (popJson.Length > 0 && !string.IsNullOrWhiteSpace(materialSupport))
                            {
                                var propList = new KidsNewShopPropBLL().GetList(s => s.MaterialSupport == materialSupport.ToUpper());
                                if (propList.Any())
                                {
                                    propList.ForEach(s => {
                                        popJson.Append("{\"Id\":\"" + s.Id + "\",\"ShopId\":\"" + shopModel.Id + "\",\"Sheet\":\""+s.PropName+"\",\"GraphicNo\":\"\",\"Gender\":\"" + s.Gender + "\",\"Quantity\":\"" + s.Quantity + "\",\"GraphicWidth\":\"\",\"GraphicLength\":\"\",\"GraphicMaterial\":\"" + s.GraphicMaterial + "\",\"PositionDescription\":\"" + s.PositionDescription + "\",\"Remark\":\"" + s.Remark + "\",\"IsValid\":\"1\",\"OrderType\":\"2\"},");
                                    });
                                }
                            }
        ...
        string GetSubjectGuidanceList()
        {
            ...
            var guidanceList = new SubjectGuidanceBLL().GetList(s => s.IsDelete == null || s.IsDelete == false).OrderByDescending(s => s.AddDate).ToList();
            ...
            {
                DateTime date1 = DateTime.Now.AddMonths(-3);
                DateTime date = new DateTime(date1.Year, date1.Month, 1);
                guidanceList = guidanceList.Where(s => s.GuidanceDate >= date).ToList();
        ...
        string AddOrder()
        {
            ...
            if (!string.IsNullOrWhiteSpace(jsonStr))
            {
                using (TransactionScope tran = new TransactionScope())
                {
                    try
                    {
                        List<HandMadeOrderDetail> orderList = JsonConvert.DeserializeObject<List<HandMadeOrderDetail>>(jsonStr);
                        if (orderList.Any())
                        {
                            ...
                            orderList.ForEach(s =>
                            {
                                ...
                                if (canSave)
                                {
                                    orderModel = new HandMadeOrderDetail();
        ...
        string EditOrder()
        {
            ...
            if (!string.IsNullOrWhiteSpace(jsonStr))
            {
                try
                {
                    HandMadeOrderDetail newOrder = JsonConvert.DeserializeObject<HandMadeOrderDetail>(jsonStr);
                    HandMadeOrderDetailBLL orderBll = new HandMadeOrderDetailBLL();
                    if (newOrder != null && newOrder.Id > 0)
                    {

                        HandMadeOrderDetail orderModel = orderBll.GetModel(newOrder.Id);
                        if (orderModel != null)
                        {
        ...
        string DeleteOrder()
        {
            ...
            if (context1.Request.QueryString["ids"] != null)
            {
                ...
                if (idList.Any())
                {
                    try
                    {
                        new HandMadeOrderDetailBLL().Delete(s => idList.Contains(s.Id));
                    }
                    catch (Exception ex)
                    {
