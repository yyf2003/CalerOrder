...
$(function () {
    ...
    $("#tbodyPOPData").delegate("input[name='txtGuidanceMonth']", "blur", function () {
        if (isPickDate == 1) {
            ...
            $.ajax({
                type: "get",
                url: "handler/AddNewShopOrder.ashx",
    ...
    $("#spanAddPOP").click(function () {
        ...
        if (shopId > 0) {
            ...
            var orderTypeText = $("#seleAddOrderType").find("option:selected").text();
    ...
    $("#spanAddPrice").click(function () {
        if (shopId > 0) {
            ...
            var orderTypeName = $("#selePriceType").find("option:selected").text();
            if (orderType == 0) {
                layer.msg('请选择费用类型');
                return false;
            }
            ...
            var payPriceNum = $.trim($("#txtPayPriceNum").val());
            if (payPriceNum == "") {
                layer.msg('请填写应付金额');
                return false;
            }
            else if (isNaN(payPriceNum)) {
                layer.msg('应付金额必须是数字');
                return false;
            }
    ...
    $("#btnImport").click(function () {
        layer.open({
            ...
            skin: 'layui-layer-rim', //加上边框
            area: ['800px', '400px'],
            content: 'ImportNewShopOrder.aspx?subjectId=' + subjectId,
            cancel: function () {
                if ($("#hfIsChange").val() == "1") {
                    $("#hfIsChange").val("");
                    Order.getShopList();
            ...
            success: function (data) {
                if (data != "") {
                    ...
                    if (orderJson.length > 0) {
                        var orderType = orderJson[0].OrderType;
                ...
                if (data != "") {
                    ...
                    if (flag == "error") {
                        ...
                        if (json.length > 0) {
                            ...
                            for (var i in json) {
                                var orderType = json[i].OrderTypeName;
                                var checked = "";
                                if (json[i].OrderType == 2) {
                                    checked = "checked='checked'";
                                }
                                tr += "<tr>";
                                tr += "<td><input type='checkbox' name='cbOnePOP' data-ordertype='" + json[i].OrderType + "' data-isvalid='" + json[i].IsValid + "' value='" + json[i].Id + "' " + checked + "></td>";
                                tr += "<td>" + orderType + "</td>";
                                tr += "<td style='padding-left:0px;'>活动：" + CreateGuidanceSelect();
                                tr += "项目：" + CreateSujectSelect() + "</td>";
                                tr += "<td>" + CreateChooseImgSelect(i) + "</td>";
    ...
    submit: function () {
        ...
        for (var i = 0; i < trs.length; i++) {
            ...
            if ($(cb).attr("checked") == "checked") {
                ...
                var orderType = $(cb).data("ordertype") || 1;
                ...
                var receivePrice = $(trs[i]).find("td").eq(8).html() || 0;
                var payPrice = $(trs[i]).find("td").eq(9).html() || 0;

                var width = $(trs[i]).find("td").eq(10).html() || 0;
                var length = $(trs[i]).find("td").eq(11).html() || 0;
                var material = $(trs[i]).find("td").eq(12).html();
                var positionDescription = $(trs[i]).find("td").eq(13).html();
...
var Prop = {
    ...
    getList: function () {
        $.ajax({
            type: "get",
            url: "handler/AddNewShopOrder.ashx?type=getPropList",
            success: function (data) {
