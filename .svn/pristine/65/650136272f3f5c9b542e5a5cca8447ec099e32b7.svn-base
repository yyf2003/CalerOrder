    ...
    public class BasicMachineFrameList : IHttpHandler
    {
        ...
        string GetMachineFrame()
        {
            ...
            if (list.Any())
            {
                ...
                int index = 1;
                list.ForEach(s => {
                    json.Append("{\"rowIndex\":\"" + index + "\",\"Id\":\"" + s.Id + "\",\"Sheet\":\"" + s.Sheet + "\",\"MachineFrame\":\"" + s.MachineFrame + "\",\"CornerType\":\""+s.CornerType+"\"},");
                    index++;
                });
                if (json.Length > 0)
            ...
            if (!string.IsNullOrWhiteSpace(jsonStr))
            {
                ...
                if (model != null)
                {
                    if (!string.IsNullOrWhiteSpace(model.MachineFrame))
                    {
                        ...
                        if (model.Id > 0)
                        {
                            if (!Check(model.Id, model.Sheet, model.MachineFrame, model.CornerType))
                            {
                                frameModel = bll.GetModel(model.Id);
                                if (frameModel != null)
                                {
                                    frameModel.MachineFrame = model.MachineFrame.ToUpper();
                                    frameModel.CornerType = model.CornerType;
                        ...
                        else
                        {
                            ...
                            int successNum = 0;
                            frameList.ForEach(s => {
                                if (!Check(0, model.Sheet, s, model.CornerType))
                                {
        ...
        bool Check(int id, string sheet, string frameName,string cornerType)
        {
